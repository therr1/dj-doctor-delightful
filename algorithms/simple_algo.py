...
class Algorithm:
    ...
    def equalizer(self):
        ...
        for i in range(NUM_LIGHTS):
            final_hex_vals[i] = hsv_to_hex_rgb_str(1.0*i/NUM_LIGHTS, 1.0, light_amplitude_values[i]*1.0/MAX_AMPLITUDE)
        # maxd = 1
    ...
    def bass_flashes(self):
        ...
        num_bass_buckets=(int)((BASS_MAX_FREQ-MIN_BUCKET)*1.0/bucket_width)
        ...
        hex_representation_of_color = hsv_to_hex_rgb_str(hue_value, 1.0, val_value)
        final_hex_vals = np.chararray(NUM_LIGHTS,itemsize=7)
        for i in range(NUM_LIGHTS):
            final_hex_vals[i] = hex_representation_of_color
        return final_hex_vals
